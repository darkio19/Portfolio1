...
(function($){
	"use strict";
	  ...
	  // Una prueba de galeria
	  $(document).ready(function () {
		$('.gallery-cover').click(function () {
			var gallerySelector = $(this).data('gallery');
			$('.' + gallerySelector).magnificPopup({
				delegate: 'a',
				gallery: {
					enabled: true
				},
				type: 'image', // Tipo predeterminado para imágenes
				image: {
					titleSrc: 'title'
				},
				callbacks: {
					elementParse: function (item) {
						// Verifica si el enlace es un video o una imagen
						if (item.src.endsWith('.mp4') || item.src.endsWith('.webm') || item.src.endsWith('.ogg')) {
							item.type = 'iframe'; // Cambia a iframe para videos
						} else {
							item.type = 'image'; // Predeterminado para imágenes
						}
					}
				}
			}).magnificPopup('open', 0); // Abre el popup inmediatamente
		});
	});
